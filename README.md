...
- **skill**
  - `caibao-analysis` — 财报数据提取→补进 `finance/` 估值库→刷新买卖点（强制「先提取再结论」，不编数据）
  - `daily-news` — 每日扫监控清单公司的新闻/公告，事实核验后追加进事实编年并出日报（见下「每日哨兵」）
  - `ingest-fraud-case` — 录入造假案例并蒸馏特征，增量喂养 `造假模式库.md`

### 模块三 · 综合估值与投资参考
选对估值法给出合理估值与买卖点，并把三个维度合成一份投研报告。

- **知识库**：`finance/<公司>/` — 唐朝估值法估值库（xlsx + 分析文档），canonical 估值数据
- **skill**
  - `valuation-method` — 估值法选择器（老唐估值法 / 完整 DCF / CAPE 周期股；不满足三前提→「无法估值」）
  - `invest-report` — **编排器**：按序调 `caibao-analysis → valuation-method → guru-view`(+行情) 合成综合投资参考报告
...
- **事实与判断分层**：`companies/` 只记发生了什么（零判断）；`finance/<公司>/分析.md` 记当前怎么看（财报来了会被 `caibao-analysis` 重刷）；`finance/<公司>/观点变更.md` 记看法怎么变（**只增不改**的历史，刷新 `分析.md` 时不动它，可回头算自己的胜率）。组合级（仓位加总/买入排队/成交流水）进入实盘阶段再建 `finance/组合/`
