...
# 科目别名(前缀匹配·A股 CAS / 港股 IFRS 中文 / **日股 IFRS 日文** 三类都覆盖)。取第一个命中的行。
# ⚠️ 2026-09-14 补日文:此前 AGC/NSG 两家日股跑出来 21 项比率全是「—」——不是漏做比率表,
#    而是别名层不认「売上高/営業利益/棚卸資産」。补别名前请先跑一遍看 unmatched 清单。
ALIASES = {
    "营收":   ["营业总收入", "营业收入", "收益 Revenue", "收益", "营业额", "売上高", "売上収益"],
    "成本":   ["营业成本", "销售成本", "减：营业成本", "売上原価"],
    "毛利":   ["毛利 ", "毛利", "売上総利益"],
    "销售费用": ["销售费用", "销售及分销开支", "减：销售费用", "销售及市场推广开支"],  # ⚠️勿加「販売費」: 日股多印合并的「販売費及び一般管理費」, 前缀匹配会让销售/管理两个比率抓到同一行、输出两个相同值
    "管理费用": ["管理费用", "行政开支", "减：管理费用", "一般及行政开支", "一般管理費", "販売費及び一般管理費"],  # 港股「行政开支」≈ A股「管理费用」
    "研发费用": ["研发费用", "减：研发费用", "研发开支", "研究開発費"],
    "净利润": ["净利润 ", "净利润", "四、净利润", "五、净利润", "年内溢利", "年度盈利", "当期純利益", "当期利益"],
    "归母":   ["归属于母公司股东的净利润", "归属于母公司所有者的净利润", "归属于上市公司股东的净利润",
              "母公司拥有人应占溢利", "归母净利润", "归母净利", "親会社の所有者に帰属する当期純利益", "親会社株主に帰属する当期純利益"],
    "扣非归母": ["扣除非经常性损益后的净利润", "归属于母公司股东的扣除非经常性损益的净利润",
              "扣非归母", "扣非净利润"],
    "税前":   ["利润总额", "三、利润总额", "除税前溢利", "税前利润", "税引前利益", "税引前当期純利益"],
    "所得税": ["所得税费用", "减：所得税费用", "所得税开支", "法人所得税費用", "法人税等"],
    ...
    "流动资产总额": ["流动资产总额", "流动资产合计", "流動資産合計"],
    "权益合计": ["所有者权益合计", "股东权益合计", "所有者权益（或股东权益）合计", "权益总额", "总权益", "資本合計"],
    "归母权益": ["归属于母公司所有者权益合计", "归属于母公司股东权益合计", "归属于母公司股东的权益合计",
              "母公司拥有人应占权益", "归母权益", "親会社の所有者に帰属する持分", "親会社所有者帰属持分"],
    "OCF":    ["经营活动产生的现金流量净额", "经营活动现金流量净额", "经营活动所得现金流量净额",
              "经营活动所得现金净额", "经营活动现金流净额", "営業活動によるキャッシュ・フロー", "営業活動によるキャッシュ"],
    "capex":  ["购建固定资产、无形资产和其他长期资产支付的现金", "购买物业厂房设备项目", "购买物业厂房设备",
              "购建固定资产", "capex", "有形固定資産の取得による支出", "有形固定資産及び無形資産の取得による支出"],
    "分红":   ["分配股利、利润或偿付利息支付的现金", "已付股息", "分配股利", "配当金の支払額", "配当金の支払"],
}
# 现金及金融资产类(前缀·求和·尽力而为)
CASH_KEYS = ["货币资金", "现金及银行结余", "现金及现金等价物", "交易性金融资产", "FVTPL金融资产",
             "长期银行存款", "结构性存款", "受限资金", "质押存款", "受限现金", "受限制现金", "定期存款",
             "债权投资", "其他债权投资"]
...
def load_table(path):
    ...
    # 年份列有两种写法：A股/港股「2025」；日股 3 月决算「FY2026/3」。
    # ⚠️ 只认前者会让日股 years 为空 → 后续 years[0] 直接 IndexError（NSG 实测）。
    years = []
    for y in raw[hdr_i][1:]:
        y = y.strip()
        if y.isdigit():
            years.append(int(y))
        else:
            m = re.match(r"^FY(\d{4})(?:/\d{1,2})?$", y, re.I)
            if m:
