...
RESTATE_ROWS = [
    ...
    # --- C 的中期落点(2026 年 10-Q 的 2025 比较列被大幅重述·年度册原只登记了年报落点) ---
    ("C 稳定币改记现金等价物", "2025H1(中期)", "经营活动现金流净额",
     "145,747 (10-Q 2025Q2 原披露·由重分类表倒推)",
     "1,092,772 (10-Q 2026Q2 比较列)", "+947,025",
     "同一政策变更在中期表的落点:营运资本合计行由 -1,036,250 调为 -89,225。"
     "**幅度达 7.5 倍** → 任何引用 2025 年当期 10-Q 原披露中期经营现金流的分析都会错量级"),
    ("C 稳定币改记现金等价物", "2025H1(中期)", "发放贷款 / 收回贷款",
     "-955,488 / 588,004 (10-Q 2025Q2)",
     "-4,596,581 / 4,322,454 (10-Q 2026Q2)", "-3,641,093 / +3,734,450",
     "稳定币借贷改按加密借贷政策后,原在营运资本内的净额流转改按**总额**列入投资活动"),
    # --- H:客户抵押品 总额→净额 列示(2026Q1 起·追溯) ---
    ("H 客户抵押品总额→净额", "2025H1(中期)", "收到客户抵押品 / 返还客户抵押品",
     "370,553 / -373,804 (10-Q 2025Q2)",
     "109,399 / -112,650 (10-Q 2026Q2·含 C 的影响)", "±312,510(仅 H 的部分)",
     "2026Q1 起将部分贷款抵押品的收与返由**总额**改**净额**列示并追溯;"
     "公司理由=周转快、金额大、期限短,与相关贷款余额的净额列示一致。"
     "融资活动净额不受影响(两行对冲),但两行的**规模感被压缩约三分之二**"),
    # --- G:经营费用「分项层」静默重分类(总费用不变,故 A/B/C 的 XBRL 侦测抓不到) ---
    ("G 费用分项静默重分类", "2023", "一般及行政 General and administrative",
     "1,041,308 (10-K FY2023)", "1,074,308 (10-K FY2024/FY2025)", "+33,000",
     "33,000 由「其他经营费用净额」重分类进「一般及行政」;**公司未在申报中给出任何说明**"
     "(FY2024 10-K 全文无 reclassified 字样)。总经营费用 3,270,045 不变 → "
     "「总费用七年零重述」仍成立,但**分项层被改过**;费用构成.csv 用最新列示,"
     "利润表.csv 保持 as-reported,两者 2023 G&A 差 33,000 即此处"),
    ("G 费用分项静默重分类", "2023", "其他经营费用净额 Other operating expense, net",
     "43,260 (10-K FY2023)", "10,260 (10-K FY2024/FY2025)", "-33,000", "同上(对侧)"),
    ("G 费用分项静默重分类", "2024", "G&A 员工相关口径(分项内)",
     "608,786「Personnel-related」(10-K FY2024)",
     "606,554「Employee-related」(10-K FY2025 ASU2024-03 附注)", "-2,232",
     "行标签由 Personnel-related 改为 Employee-related,2,232 移入「其他」;G&A 合计不变"),
...
def write_restate():
    ...
    with open(p, "w", newline="") as f:
        ...
        w.writerow(["⚠️ 断点 G 是**分项层**重分类(合计不变),XBRL 按合计科目比对抓不到 —— "
                    "只能靠印刷报表逐行对照发现;费用构成.csv 用最新列示,利润表.csv 保持 as-reported"])
