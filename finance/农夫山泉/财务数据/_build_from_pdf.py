...
利润表 = [
    ...
    ("每股基本及摊薄盈利(元) Basic and diluted EPS",    [0.31, 0.33, 0.46, 0.48, 0.64, 0.76, 1.074, 1.078, 1.411]),
...
# ====== 派生比率 ======
def build_ratios():
    # ── 通用底 ← scripts/derived.py（全公司共有比率·单一逻辑·跨公司复用）
    import sys
    ...
    # ── 定制层（农夫/港股特有）：扣非净利率(估) —— 港股无标准扣非线, 剔投资类非经常(税后)估算
    ...
    def adj_margin(i):  # 扣非归母(估) = 归母 + (1-有效税率)×Σ(投资类非经常 CF addback), ÷营收
        trate = safe_div(-tax[i], pbt[i]) or 0
        s = sum(x[i] for x in nr if x[i] is not None)
        return safe_div(parent[i] + (1 - trate) * s, rev[i])
    kf_row = [adj_margin(i) for i in range(N)]

    # 通用底(取 名+值列) + 把 derived 的「扣非净利率(估) n/a」替换成农夫自估值
    rows = []
    for name, vals, _fmt in common:
        rows.append((name, kf_row if name.startswith("扣非净利率") else vals))
    return rows
